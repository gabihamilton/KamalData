...
// Wed Mar  8 13:57:35 2023 by ROOT version 6.26/06
// from TTree tree/all observables, signal
// found on file: tree_VBFG_mG2000_v3_MC2016.root
//////////////////////////////////////////////////////////
...
#ifdef AnalyseVBF_cxx
AnalyseVBF::AnalyseVBF(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("tree_VBFG_mG2000_v3_MC2016.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("tree_VBFG_mG2000_v3_MC2016.root");
      }
...
AnalyseVBF::~AnalyseVBF()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t AnalyseVBF::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t AnalyseVBF::LoadTree(Long64_t entry)
{
...
void AnalyseVBF::Init(TTree *tree)
{
...
void AnalyseVBF::Show(Long64_t entry)
{
...
Int_t AnalyseVBF::Cut(Long64_t entry)
{
